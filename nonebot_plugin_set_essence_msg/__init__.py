...
from nonebot.adapters.onebot.v11 import Bot, GroupMessageEvent  # 改用 GroupMessageEvent
...
def _get_reply_msg_id(
    event: GroupMessageEvent,
) -> int | None:  # 明确类型为 GroupMessageEvent
    """获取被回复的消息ID"""
    if event.reply:
        try:
            return int(event.reply.message_id)  # type: ignore
    ...
    for seg in event.message:
        if seg.type == "reply":
            try:
                return int(seg.data.get("id", 0))  # 提供默认值避免 None
            except (ValueError, TypeError):
                pass
    return None
...
@set_cmd.handle()
async def _handle_set(bot: Bot, event: GroupMessageEvent):  # 使用 GroupMessageEvent
    ...
    try:
        ...
        logger.info(
            f"设精成功: group={event.group_id}, msg={msg_id}, op={event.user_id}"
        )
...
@remove_cmd.handle()
async def _handle_remove(bot: Bot, event: GroupMessageEvent):  # 使用 GroupMessageEvent
    msg_id = _get_reply_msg_id(event)
    if not msg_id:
    ...
    try:
        ...
        logger.info(
            f"移精成功: group={event.group_id}, msg={msg_id}, op={event.user_id}"
        )
